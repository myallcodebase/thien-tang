...
const EXCLUDED_LOGOS = ["Bulma", "Gulp", "Unity", "Jira", "Confluence", "Figma", "Grunt"];

const LogoSlider = ({ logos, title }: LogoSliderProps) => {
  const [showMore, setShowMore] = useState(false);
  const filteredLogos = logos.filter(({ name }) => !EXCLUDED_LOGOS.includes(name));

  const renderLogoCarousel = (): JSX.Element => (
    <ul>
      {filteredLogos.map(({ id, logo: { url }, name }) => (
        <LogoSliderItem height={60} width={60} imgSrc={url} technologyName={name} key={id} />
      ))}
    </ul>
  );

  const lessLogos = filteredLogos.slice(0, MIN_LOGOS_TO_SHOW);

  const logosToRender = showMore ? filteredLogos : lessLogos;

  const calculatedWidth = filteredLogos.length * LOGO_WIDTH;
